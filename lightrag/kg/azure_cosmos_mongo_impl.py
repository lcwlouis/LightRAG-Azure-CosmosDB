...
@final
@dataclass
class AzureCosmosMongoKVStorage(BaseKVStorage):
...
@final
@dataclass
class AzureCosmosMongoDocStatusStorage(DocStatusStorage):
    db: AsyncIOMotorDatabase = field(default=None)
    _data: AsyncIOMotorCollection = field(default=None)
...
@final
@dataclass
class AzureCosmosMongoGraphStorage(BaseGraphStorage):
    """
    A concrete implementation using Azure Cosmos DB for MongoDB vCore's $graphLookup to demonstrate multi-hop queries.
    """
                            ...
                            result.nodes.append(
                                KnowledgeGraphNode(
                                    ...
                                    properties={
                                        ...
                                        if k not in ["_id", "edges", "connected_nodes"]
                                    },
                                )
                            )
...
@final
@dataclass
class AzureCosmosMongoVectorDBStorage(BaseVectorStorage):
    ...
    async def create_vector_index_if_not_exists(self):
        """Creates a Vector Search index using HNSW."""
        try:
            index_name = "vector_knn_index"

            # Check if index already exists
            indexes = await self._data.list_indexes().to_list(length=None)
            ...
            logger.info("Vector index created successfully using HNSW.")
    ...
    async def query(
        ...
        """Queries the vector database using Vector Search with HNSW."""
        ...
        pipeline = [
            {
                "$search": {
                    "cosmosSearch": {
                        ...
                        "path": "vector",
                        "k": top_k,
                        "efSearch": 40,  # HNSW specific parameter for search accuracy
                    },
        ...
        return [
            {
                **doc["document"],
                "id": doc["document"]["_id"],
                "distance": doc.get("similarityScore", None),
                "created_at": doc["document"].get("created_at"),
            }
            for doc in results
        ]

    async def index_done_callback(self) -> None:
        # Cosmos DB handles persistence automatically
        pass
